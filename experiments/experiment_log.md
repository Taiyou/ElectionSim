...
- **モデル**: anthropic/claude-sonnet-4, temperature=0.7
- **成功基準**:
  - 投票率が55±5%に収まる（v4a: 75.5%, v7a: 88.7%）
  - chudo議席がv4aより減少（中道バイアス軽減）
  - v1との一致率 > 70%
- **実行コマンド**: `python scripts/run_v8_calibrated_vote.py --mode pilot --seed 42`
- **結果**: (実行後に記入)
...
- **方法**: 安定区はルールベース（v2と同一）、接戦区はv8aデカップリング方式
- **モデル**: anthropic/claude-sonnet-4, temperature=0.7
- **実行コマンド**: `python scripts/run_v9_hybrid_ensemble.py --seed 42`
- **結果**: (実行後に記入)
